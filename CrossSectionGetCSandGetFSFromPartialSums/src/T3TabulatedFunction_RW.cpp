...
template<class T1, class T2> size_t T3TabulatedFunction<T1, T2>::Get_size() const
{
  ...
  {
    T3cout << "T3IsotopeFission::Get_size Error: numbers of arguments and values differ: "
...
template<class T1, class T2> bool T3TabulatedFunction<T1, T2>::Is_sorted() const
{
  bool result = true;
...
#ifdef debug
    if (arg < Get_max_arg())
      T3T3cout << "T3TabulatedFunction::Add_point: Warning: non-ascending argument order, "
                                                                      ...
                                                                      T2 by_def) const
{
  ...
  else
  {
    ...
    const double val_low = Get_val(ind_low);
    const double val_upp = Get_val(ind_upp);
#ifdef debug
    T3cout << "T3TabulatedFunction::Calc_xs(" << arg << "): E:xs(ind_low=" << ind_low
...
#ifdef DEBUG
  T3cout << "T3TabulatedFunction<T1, T2>::Save_to: " << *this;
  T3cout << "T3TF::Save_to: size " << this->Get_size() << " " << vector_size
  << "sofT1="<<sizeof(T1) << " sofT2="<<sizeof(T2)<< T3endl;
#endif
  //
  T3cout<<"vector_size="<<vector_size<<T3endl;
  std::cout<<"arg: ";
  for(int i=0; i<Get_size(); ++i) T3cout<<_arg.at(i)<<" ";
  T3cout<<T3endl;
  std::cout<<"val: ";
  for(int i=0; i<Get_size(); ++i) T3cout<<_val.at(i)<<" ";
  T3cout<<T3endl;
  //check os:
  T3cout<<"Check os:"<<T3endl;
  std::cout<<"1: "<<std::endl;
  //
...
#ifdef DEBUG
  T3cout << "T3TabulatedFunction<T1, T2>::Load_from: " << *this;
  T3cout << "T3TF::L_f: size " << this->Get_size() << " " << vector_size
  << "sofT1="<<sizeof(T1) << " sofT2="<<sizeof(T2)<< T3endl;
#endif
}
...
template class T3TabulatedFunction<double, double>;
