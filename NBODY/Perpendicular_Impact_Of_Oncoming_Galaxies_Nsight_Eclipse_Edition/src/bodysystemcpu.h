...
{
    T posx[10]; // mapped host pointers
    T posy[10];
    T posz[10];
    positionInTime()
    {
    	memset(posx, 0, 10*sizeof(T));
    	memset(posy, 0, 10*sizeof(T));
    	memset(posz, 0, 10*sizeof(T));
    }
};*/
template <typename T>
struct positionInTime
{
    T *posx; // mapped host pointers
    T *posy;
    T *posz;
    positionInTime()
    {
    	posx = new T[numIterations];
    	posy = new T[numIterations];
    	posz = new T[numIterations];
    	memset(posx, 0, numIterations*sizeof(T));
    	memset(posy, 0, numIterations*sizeof(T));
    	memset(posz, 0, numIterations*sizeof(T));
    }
};
