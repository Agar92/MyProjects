...
int numIterations = 500; // run until exit
                      ...
                      float milliseconds, int iterations)
{
    ...
    // measure of "science throughput". We are usnumBodiesing it as a measure of
...
{
    { 0.0005f, 10.f, 0.008f, 0.000001f, 1.0f, 1.0f, 0, -2, -100}/*,MASS/r
    ...
    public:
        ...
        static void display()
        {
            ...
            if (useHostMem)
            {
                ...
                // writing.  If we don't wait until CUDA is dTRUEone updating it, we will render partially
        ...
        static void makeVRML()
		{
				...
				fout<<"Group {\n"<<"children [\n";
				fout<<"DEF VIEW Viewpoint { position 0 0 300 orientation 0 -1 0 0 }\n";
				for(int i=0; i< numBodies; ++i)
				{
					if(i==0 || i==numBodies/2)
					{
					  fout<<"DEF Atom"<<i<<" Transform {\n"<<"translation "<<NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posx[0]<<" "<<NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posy[0]<<" "<<NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posz[0]<<"\n";
					  fout<<"center "<<-NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posx[0]<<" "<<-NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posy[0]<<" "<<-NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posz[0]<<"\n";
					  //fout<<"center "<<0<<" "<<0<<" "<<0<<"\n";
				...
				for(int i=0; i< numBodies; ++i)
				{
					...
					{
						fout<<k*1.0/(numIterations-1)<<" ";
					}
					fout<<"]\n"<<"keyValue [ ";
					for(int j=0; j<numIterations; ++j) fout<<NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posx[j]<<" "<<NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posy[j]<<" "<<NBodyDemo<float>::m_singleton->m_nbodyCpu->vrmlpos[i].posz[j]<<"\n";
					fout<<"\n]\n}";
        ...
        void _reset(int numBodies, NBodyConfig config)
        {
            if (tipsyFile == "")
            {
                randomizeBodies(config, m_hPos, m_hVel, m_hColor,
                                ...
                                numBodies, activeParams.m_damping , true);
