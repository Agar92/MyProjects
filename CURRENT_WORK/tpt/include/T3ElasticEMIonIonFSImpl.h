...
public:
  ...
  inline Floating GetRNDCosThetaCMInEqRutherford(
                  Floating mr/*reduced mass*/, int z,
                  Floating pcm2,
                  Floating tmin/*|t|min*/,
                  Floating tmax/*|t|max*/,
                  Floating tup/*=|t|max-|t|min or k*|t|m*/,
...
private:
  ...
  //(the partial sums are at k -:- 1.0)
  const Floating k=1.0e-3;
...
Floating T3ElasticEMIonIonFS<Floating>::GetRNDCosThetaCMInEqRutherford(
...
//There are 3 channels for equal particles Rutherford scattering.
  ...
  const CS1=(1.0/tmin-1.0/tup) * COEF*hc*hc;
  const CS2=(1.0/(tmax-tup)-1.0/(tmax-tmin)) * COEF*hc*hc;
  //Find spin:
  ...
  const CS3=SIGN*2.0/(2*spin+1)/tmax*beta_r_cm/alpha*
                                        (cs3factor1-cs3factor2) * 2*COEF*hc*hc;
...
//3. Find |t|min:
  const Floating M=aParticleTabe.GetMass(isotopePDG);//mass of the target ion.
...
//7. Get random cos(theta_cm):
  ...
  {
    //D-D elastic scattering using at Tls >= 30 keV
    //|t|min <= |t| <= 10^(-3)*|t|m:
    if(incPDG == deuteronPDG && Tls >= 30*keV)
      GetRNDCosThetaCMInEqRutherford(mr,z,pcm2,tmin,tmax,
                                     k*tm,R1,R2,aParticleTable);
    //1) D-D, but Tls < 30 keV; 2) NOT D-D, Ti-Ti or other:
    else
      GetRNDCosThetaCMInEqRutherford(mr,z,pcm2,tmin,tmax,
