...
T3Inelasticdd_DB & T3Inelasticdd_DB::operator=(T3Inelasticdd_DB const & nodedb)
{
  for(int i=0; i<512; ++i)
  {
  ...
  memcpy(this->pr, nodedb.pr, _num_point * sizeof(T3double));
  memcpy(this->sl, nodedb.sl, _num_point * sizeof(T3double));
  Xmin=nodedb.Xmin;
  Xmax=nodedb.Xmax;
  return (*this);
}
...
//#pragma acc routine seq
T3double T3Inelasticdd_DB::RandomizeCost(unsigned int & generator_seed, T3double E) const
{
