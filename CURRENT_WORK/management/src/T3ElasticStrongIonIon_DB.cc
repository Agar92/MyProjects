...
T3ElasticStrongIonIon_DB & T3ElasticStrongIonIon_DB::operator=(T3ElasticStrongIonIon_DB const & nodedb)
{
  for(int i=0; i<512; ++i)
  {
  ...
  memcpy(this->lncoscm, nodedb.lncoscm, _num_point * sizeof(T3double));
  deltaLnCosThetaCM=nodedb.deltaLnCosThetaCM;
  Xmin=nodedb.Xmin;
  Xmax=nodedb.Xmax;
  return (*this);
}
...
//HERE cost MUST BE FROM 0.0 TO 1.0-1.0e-3=0.999.!!!
//!!!  
