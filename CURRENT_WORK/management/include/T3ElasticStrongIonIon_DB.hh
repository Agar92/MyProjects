...
{
public:
  T3ElasticStrongIonIon_DB(T3double xmin=-1.0, T3double xmax=1.0):Xmin(xmin),Xmax(xmax)
  {
    Xmin=std::log(CosThetaCMmin);
    Xmax=std::log(CosThetaCMmax);
    deltaLnCosThetaCM=(Xmax-Xmax)/(_num_point+1);
    //we have 129 values of 1-costhetacm: 0 ... 1 and 128 bins.
    //but we fill lncoscm only with 127 values excluding 0 and 1:
    for(int i=0; i<127; ++i)
      lncoscm[i]=Xmin+(i+1)*deltaLnCosThetaCM;
  }
  T3ElasticStrongIonIon_DB & operator=(T3ElasticStrongIonIon_DB const & nodedb);
