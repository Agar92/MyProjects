...
//\\//*********************************//
//This is not necessary (used nowhere),
...
//  pnames[2112] = "n";
  char buffer[1023];
  T3String T2data;                // Check that the NNG data exist
  if (getenv("T2_DATA"))
  {
  ...
  else
  {
    ...
    T3cout << "-Warning-T3NSGangular_RW::default_file:Cant't get $T2_DATA. Use ./data"
           << T3endl;
  }
