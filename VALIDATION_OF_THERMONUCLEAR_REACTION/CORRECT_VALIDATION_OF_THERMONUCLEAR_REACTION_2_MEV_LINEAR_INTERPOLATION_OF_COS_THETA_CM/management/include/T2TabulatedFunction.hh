#pragma once
#ifndef T2TABULATEDFUNCTION_HH
#define T2TABULATEDFUNCTION_HH 1
...
template<class T1, class T2> class T2TabulatedFunction
{
public:
  T2TabulatedFunction(std::vector<T1> arg = std::vector<T1>(),
  ...
  const T2TabulatedFunction<T1, T2> operator*(const T1& t1) const;
  const T2TabulatedFunction<T1, T2> operator+(const T2TabulatedFunction<T1, T2>& t) const;
  G4bool operator==(const T2TabulatedFunction<T1, T2>& tab) const;
private:
  //Body
  std::vector<T1> _arg;
  std::vector<T2> _val;
...
std::ostream& operator<<(std::ostream& os, const T2TabulatedFunction<T1, T2>& inst)
{
