...
template<class T1, class T2> size_t T2TabulatedFunction<T1, T2>::Get_size() const
{
#ifdef debug
  if( _arg.size() != _val.size() )
  {
    G4cout << "T2IsotopeFission::Get_size Error: numbers of arguments and values differ: "
           << "#arg = " << _arg.size() << ", #val = " << _val.size() << G4endl;
    rexit(-1);
  }
#endif
  return _arg.size();
...
template<class T1, class T2> G4bool T2TabulatedFunction<T1, T2>::Is_sorted() const
{
  G4bool result = true;
...
void T2TabulatedFunction<T1, T2>:: Erase(size_t ind_min, size_t ind_after_max)
{
  if(ind_min >= ind_after_max)
  {
    G4cout << "Warning: wrong index order: " << ind_min << "<=" << ind_after_max << G4endl;
  }
  else if(ind_min >= Get_size())
  {
    G4cout << "Warning: min_index out of range: " << ind_min << "=>" << Get_size()
           << G4endl;
  }
  else if(ind_after_max > Get_size())
  {
    G4cout << "Warning: min_index out of range: " << ind_after_max << ">" << Get_size()
           << "Erasing to the end" << G4endl;
...
template<class T1, class T2> void T2TabulatedFunction<T1, T2>::Add_point(T1 arg, T2 val,
                                                                           T1 delta)
{
  ...
  else
  {
#ifdef debug
    if (arg < Get_max_arg())
      G4cout << "T2TabulatedFunction::Add_point: Warning: non-ascending argument order, "
             << " max_arg = " << Get_max_arg() << ", new arg = " << arg << G4endl;
#endif
    size_t index = 0;
    ...
    {
#ifdef debug
      G4cout << "Double-value point arg = " << arg << G4endl;
#endif
...
template<class T1, class T2> T2 T2TabulatedFunction<T1, T2>::Calc_val(T1 arg,
                                                                      T2 by_def) const
{
...
//     G4cout << ind_upp << ' ' << ind_low << ' ' << Get_size() << G4endl;
    ...
    const G4double val_low = Get_val(ind_low);
    const G4double val_upp = Get_val(ind_upp);
#ifdef debug
    G4cout << "T2TabulatedFunction::Calc_xs(" << arg << "): E:xs(ind_low=" << ind_low
    << ") = " << arg_low << " : " << val_low << ", E:xs(ind_upp="
    << ind_upp << ") = " << arg_upp<< " : " << val_low << G4endl;
#endif
...
template<class T1, class T2> const T2TabulatedFunction<T1, T2>
T2TabulatedFunction<T1, T2>::operator*(const T1& t1) const
{
  T2TabulatedFunction<T1, T2> result;
...
template<class T1, class T2> const T2TabulatedFunction<T1, T2>
T2TabulatedFunction<T1, T2>::operator+(const T2TabulatedFunction<T1, T2>& t) const
{
  T2TabulatedFunction<T1, T2> result;
  size_t ind1 =0;
  size_t ind2 = 0;
...
T2TabulatedFunction<T1, T2>::Get_equidistant(T1 arg_min, T1 arg_max, size_t nsteps) const
{
...
template<class T1, class T2>
void T2TabulatedFunction<T1, T2>::Save_to(std::ostream& os) const
{
  unsigned int vector_size = Get_size();
#ifdef DEBUG
  G4cout << "T2TabulatedFunction<T1, T2>::Save_to: " << *this;
  G4cout << "T2TF::Save_to: size " << this->Get_size() << " " << vector_size
  << "sofT1="<<sizeof(T1) << " sofT2="<<sizeof(T2)<< G4endl;
#endif
...
template<class T1, class T2>
void T2TabulatedFunction<T1, T2>::Load_from(std::istream& is)
{
...
#ifdef DEBUG
  G4cout << "T2TabulatedFunction<T1, T2>::Load_from: " << *this;
  G4cout << "T2TF::L_f: size " << this->Get_size() << " " << vector_size
  << "sofT1="<<sizeof(T1) << " sofT2="<<sizeof(T2)<< G4endl;
#endif
}

template<class T1, class T2> T2TabulatedFunction<T1, T2>&
T2TabulatedFunction<T1, T2>::operator=(const T2TabulatedFunction<T1, T2>& tab)
{
  _arg = tab._arg;
  _val = tab._val;
  return *this;
...
template<class T1, class T2> G4bool
T2TabulatedFunction<T1, T2>::operator==(const T2TabulatedFunction<T1, T2>& tab) const
{
  return _arg == tab._arg && _val == tab._val;
}

template class T2TabulatedFunction<G4double, G4double>;
